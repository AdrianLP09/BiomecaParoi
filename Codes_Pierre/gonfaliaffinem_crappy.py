...
#à peu près carré si pas Mullins pour p=2800 et i = 2000
  ...
  ver = crappy.blocks.VideoExtenso(camera='XiAPI',
                                   config=True, 
                                   save_images=True,
                                   save_folder=f'./'+date+'/40d_cd/p{p}i{i}pente{pente}o{o}O{O}/right/',
  ...
  vel = crappy.blocks.VideoExtenso(camera='XiAPI', 
                                   config=True,
                                   save_images=True,
                                   save_folder=f'./'+date+'/40d_cd/p{p}i{i}pente{pente}o{o}O{O}/left/',
  ...
  trid = tridicalddeflin.Tridical(direct_A_file='./'+date+'/40d_cd/coeff_direct.npy',
                                  transfomatrix_file='./'+date+'/40d_cd/transfomatrix.npy',
                                  label = 'exx')
                            
  trig_cam = trigger.Triggercamexxlin(cmd_labels=['exx', 'exxcmd'])
  
  trig_ali = trigger.Triggerali(Pseuil=6,
                                Miniflow=7,
                                cmd_labels=['exx', 'press'])
  ...
  gen_exx = crappy.blocks.Generator([{'type': 'Constant',
                                     ...
                                     {'type': 'Ramp',
                                      'speed': pente,
                                      'condition': 'delay=100',
                                      'init_value': 0.025}], cmd_label='exxcmd', spam=True)                                 
  ...
  pid = crappy.blocks.PID(kp=p,
                          ...
                          out_max=O,
                          out_min=o,
                          setpoint_label='exxcmd',
                          input_label='exx',
                          labels=['t(s)', 'flowcmd'],
                          debug=True)
  
  rec_tridexx = crappy.blocks.Recorder(file_name=f'./'+date+'/40d_cd/p{p}i{i}pente{pente}o{o}O{O}/defexxsvp')
  
  rec_pid = crappy.blocks.Recorder(file_name=f'./'+date+'/40d_cd/p{p}i{i}pente{pente}o{o}O{O}/pid')  

  rec_ali = crappy.blocks.Recorder(file_name=f'./'+date+'/40d_cd/p{p}i{i}pente{pente}o{o}O{O}/ali')

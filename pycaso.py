...
def Soloff_calibration (z_list : np.ndarray,
    ...
    try :
        ...
        save_calib = kwargs['saving_folder']+'3D_coordinates/'
    except :
        save_retro = 'Pycaso_retroprojection_error/'
        save_calib='3D_coordinates/'
    if not os.path.exists(save_retro) :
        P = pathlib.Path(save_retro)
        pathlib.Path.mkdir(P, parents = True)
    if not os.path.exists(save_calib) :
        P = pathlib.Path(save_calib)
        pathlib.Path.mkdir(P, parents = True)
    z_recover = z_list*1
    try :    
        for it in range(iterations) :
            ...
            np.save(save_calib + '3D_coordinates_Soloff.npy', x)
...
def Lagrange_calibration (z_list : np.ndarray,
    ...
    try :
        for it in range(iterations) :
            ...
            # each camera i
            x, Xc1, Xc2 = data.camera_coordinates(all_X, all_xth, z_points)
            np.save(save_calib + '3D_coordinates_Lagrange.npy', x)
...
def Zernike_calibration (z_list : np.ndarray,
    ...
    try :
        for it in range(iterations) :
            ...
            # each camera
            x, Xc1, Xc2 = data.camera_coordinates(all_X, all_xth, z_points)
            np.save(save_calib + '3D_coordinates_Zernike.npy', x)
